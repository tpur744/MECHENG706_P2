...
//   v4: phototransistors smoothed via EMA (updatePT), all reads use
//       globals s1–s4 instead of raw analogRead().
// =====================================================
...
const float FRONT_IR_OBSTACLE_THRESHOLD = 1.3;
...
enum State {
  ...
  ALIGNING,             // servo alignment: maximise inner sensors (2+3)
...
struct AlignState {
  ...
  int bestInnerSum = 0;           // maximise inner sensors (s2+s3)
...
void doStrafe() {
  ...
  if (state == STRAFE_FORWARD) {
    if (obstacleAhead) {
      // Obstacle back in view — abort burst, go back to strafing
  ...
  if (obstacleAhead) {
    ...
    strafe.clearCount++;
    if (strafe.clearCount >= 4) {  // ~300ms of clear readings at 20ms loop
      strafe.clearCount = 0;
...
// =====================================================
// ALIGN — servo sweep to maximise inner sensors (s2+s3) on the light
// =====================================================
void doAlign() {
  updatePT();
...
void driveForward() {
  ...
  // Smoothed threshold — lower than raw 950 because EMA pulls peaks down
  const int thr = 980;
