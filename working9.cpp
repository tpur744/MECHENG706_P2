// =====================================================
// HEAD TO LIGHT — redesigned v6
//   All v5 features retained.
//   UPDATED: STRAFE_RESCAN now rotates in the opposite
//        direction to the strafe (light is always on the
//        side we strafed away from). Sweep widened from
//        120° to 180° since we're searching a known half.
// =====================================================
...
const float SIDE_IR_OBSTACLE_THRESHOLD = 2; 
...
//   s1–s4 are globals; call updatePT() each tick.
...
enum State {
  ...
  STRAFE_RESCAN,        // phase 4: rotate, read PTs, lock onto peak heading  ← NEW
  RESCAN_SWEEP,         // phase 5: rotate through ±60° arc (post-drive rescan)
  RESCAN_ROTATE,        // phase 6: rotate to best heading found
...
void showStateOnLED(int s) {
  ...
  switch (s) {
    ...
    case STRAFE_RESCAN:   o=1;      b=1;      break;  // orange+blue  ← NEW
...
struct StrafeState {
  ...
  // ── STRAFE_RESCAN ──────────────────────────────────────────────────────
  float rescanStartHeading = 0;  // heading when rescan rotation began
  float rescanBestHeading  = 0;  // heading of best PT sum seen so far
  int   rescanBestSum      = 0;  // best PT sum seen so far
  int   rescanLastSum      = 0;  // previous tick's sum (for peak detection)
  int   rescanDropCount    = 0;  // consecutive ticks sum has dropped below peak
  int   rescanDir          = 1;  // rotation direction: +1 or -1
};
StrafeState strafe;
...
const unsigned long POST_STRAFE_COAST_MS    = 0;
const unsigned long STRAFE_FORWARD_BURST_MS = 250;

// =====================================================
// RESCAN STATE  (post-drive, not strafe)
// =====================================================
struct RescanState {
  float sweepStart       = 0;
...
void loop() {
  ...
  switch (state) {
    ...
    case STRAFE_RESCAN:  doStrafeRescan(); break;   // ← NEW
...
void doStrafe() {
  ...
  // ── Phase 3: forward burst ───────────────────────────────────────────────
  if (state == STRAFE_FORWARD) {
    if (obstacleAhead) {
      // Obstacle back — abort burst, back to strafing
    ...
    if (now - strafe.forwardStart >= STRAFE_FORWARD_BURST_MS) {
      ...
      // Light is on the side we strafed AWAY from, so rotate opposite to strafe
  ...
  if (state == STRAFE_COAST) {
    ...
    if (sideIRObstacleDetected(strafe.dir)) {
      ...
      Serial.println("STRAFE: side IR hit during coast — aborting");
  ...
  if (side == -1) {
    ...
    if (sideIRObstacleDetected(strafe.dir)) {
    stopMotors(); delay(100);
    Serial.println("STRAFE: side IR hit — aborting strafe");
    strafe.clearCount   = 0;
    strafe.forwardStart = millis();
    state = STRAFE_FORWARD;   // skip coast, go straight to forward burst
    return;
  }
...
// =====================================================
// STRAFE RESCAN — phase 4
//   Rotate slowly, read PTs each tick.
//   Lock onto the heading where PT sum peaks, then
//   hand off to ROTATING → DRIVING.
// =====================================================
void doStrafeRescan() {
  updateGyro();
  updatePT();
  ...
  if (sum > strafe.rescanBestSum) {
    ...
    strafe.rescanBestHeading = heading;
    strafe.rescanDropCount   = 0;
  } else if (strafe.rescanBestSum > 800 && sum < strafe.rescanBestSum - 150) {
    // Sum has meaningfully dropped after a real peak — we've passed it
    strafe.rescanDropCount++;
  }

  // Peak confirmed after 3 consecutive dropping ticks (~60ms at 20ms loop)
  if (strafe.rescanDropCount >= 3) {
    stopMotors(); delay(100);
  ...
  // (180° is enough — we know which half-space the light is in)
...
// =====================================================
// RESCAN  (post-drive, not post-strafe)
// =====================================================
void doRescan() {
  rescan.sweepBestSum = bestDriveSum;
  updateGyro();
...
void doAlign() {
  ...
  if (innerSum > alignSt.bestInnerSum) {
...
void driveForward() {
  ...
  float maxSensor   = max(max(s1, s2), max(s3, s4));
  bool mid_lights_high = (s2 + s3) > 1850;
  // bool outer_lights_lowish = (s1 < 875) || (s4 < 875);
  bool  lightInView = mid_lights_high;
  bool  frontIRClose    = frontIRObstacleDetected();
  bool  ultrasonicClose = (dist < 10.0);
